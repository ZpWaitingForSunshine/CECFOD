...
from tools import split_array_by_greater_than_N, calu_distance, calu_fly_power_cost
    ...
    def aimFunc(self, pop):  # 目标函数
        ...
        for n in range(len(groups)):  # i表示group的无人机的id
            ...
            for r in range(rn):  # 现在是一个无人机飞行的操作
                ...
                # if r < rn - 1:
                #     task.nextNode = task_groups[i][r + 1]
                ...
                for i in range(settings.I):  # I
                    ...
                    if task_n_schedule_matrix[i][task.n] == 1 or num_parallelism == 0: # 表明在本地制作的
                        ...
                        if i == 3:  # 更新C  需要 等所有的
                            ...
                            subTask.submitTime = max(task.quene[0] * task.subTasks[0].finished_time,
                                                        task.quene[1] * task.subTasks[1].finished_time,
                                                        task.quene[2] * task.subTasks[2].finished_time)
                        ...
                        if i == 1:  # 如果MSI在云端，需要上传
                            if task.quene[0] == 0: # 第一个也是在云端
                                ...
                                subTask.submitTime = task.photoTime
                                task.data_c['MSI'] = 1  # 变为缓存
                            subTask.execution_time_c = settings.RDMT_Cloud[i, int(num_parallelism)]
                            subTask.finished_time = subTask.submitTime + subTask.communication + subTask.execution_time_c
                        ...
                        if i == 3:  # 是S的更新
                            ...
                            if task.data_c['S'] == 0:  # 没有缓存, S在云上
                                ...
                                task.data_c['S'] = 1  # 变为缓存
                            subTask.submitTime = max((1 - task.quene[0]) * task.subTasks[0].finished_time,
                                                        (1 - task.quene[1]) * task.subTasks[1].finished_time,
                                                        (1 - task.quene[2]) * task.subTasks[2].finished_time)
                            subTask.finished_time = subTask.submitTime + subTask.communication + subTask.execution_time_e  # + 通讯时间和执行时间 就是结束时间
                            task.data_c['C'] = 1
                ...
                # todo  放到云列表上的subtasks的任务， 这个是按照消耗时间排列的，
